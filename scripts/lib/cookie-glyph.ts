...
import { type Canvas, type Harmonic, hex, sdCircle, sdBlob, sdRing, coverage, soft } from './raster';

const COOKIE = hex('#E8A94E');
const CHIP = hex('#5A3722');
const SHADOW: [number, number, number] = [17, 12, 8];
...
const COOKIE_HARMONICS: Harmonic[] = [
...
const CHIPS: Chip[] = [
  { dx: -0.2, dy: -0.22, r: 0.085, harmonics: [{ amp: 0.12, freq: 4, phase: 0.3 }, { amp: 0.07, freq: 3, phase: 1.1 }] },
  { dx: 0.14, dy: -0.28, r: 0.07, harmonics: [{ amp: 0.11, freq: 3, phase: 2.0 }, { amp: 0.06, freq: 5, phase: 0.5 }] },
  { dx: 0.3, dy: -0.02, r: 0.075, harmonics: [{ amp: 0.13, freq: 4, phase: 1.4 }, { amp: 0.06, freq: 3, phase: 2.6 }] },
  ...
  { dx: 0.02, dy: 0.3, r: 0.07, harmonics: [{ amp: 0.11, freq: 3, phase: 1.8 }, { amp: 0.06, freq: 4, phase: 3.4 }] },
  { dx: -0.24, dy: 0.28, r: 0.06, harmonics: [{ amp: 0.12, freq: 4, phase: 0.2 }, { amp: 0.07, freq: 3, phase: 2.9 }] },
];

export interface DrawCookieGlyphOptions {
  cx: number;
  cy: number;
  /** The glyph's full width/height — the cookie radius is 0.44 * size. */
  size: number;
  shadow?: boolean;
}

export function drawCookieGlyph(canvas: Canvas, { cx, cy, size, shadow = true }: DrawCookieGlyphOptions): void {
  const r = size * 0.44;
  ...
  const ring = (ox: number, oy: number, angle: number) => (x: number, y: number) =>
